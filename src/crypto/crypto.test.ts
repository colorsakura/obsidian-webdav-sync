...
import { isEncrypted, HEADER_SIZE, ENCRYPTION_OVERHEAD, packHeader, unpackHeader } from '~/crypto/file-header'
...
describe('End-to-End Sync Flow', () => {
  ...
  it('ConflictResolve: merge two encrypted versions', async () => {
    ...
    const remoteEncrypted = await encrypt(str2ab('version B: line 1\nline 2 edited'), key)
  ...
  it('LOOSE mode: encrypted remote size correctly matches local size after subtracting overhead', async () => {
    ...
    expect(encrypted.byteLength - ENCRYPTION_OVERHEAD).toBe(localContent.byteLength)

export default {
	...
	settings: {
		...
		cache: {
			...
			dumpName: '缓存管理',
			dumpDesc:
				'插件会在您的设备上保存远程文件夹的目录信息。当您更换设备时，这些信息会丢失，需要在首次同步前重新获取。如果您的文件数量较多，可能会触发坚果云的访问频率限制，导致获取过程变慢。导出功能可将这些信息保存到远程服务器，导入功能则可将数据恢复到新设备，使您无需等待即可直接同步。',
			dump: '导出',
			restoreName: '导入缓存',
			restoreDesc:
			...
			clearName: '清除本地缓存',
			clearDesc:
				'删除当前设备上的本地缓存数据。此操作无法撤销，会导致下次同步前需要重新建立缓存。仅影响当前设备，不会影响远程服务器上的数据。',
			clear: '清除',
			confirm: '确认清除',
			cleared: '缓存已成功清除',
			clearModal: {
	...
	sync: {
		...
		error: {
			...
			accountNotConfigured: '尚未配置 WebDAV 连接，请先在设置中配置连接信息',
		},
		...
		suggestUseClientForManyTasks:
			'提示：当同步任务较多时，建议使用桌面同步客户端，可获得更好的性能和稳定性，插件更适合在移动端使用！',
		modalTitle: '同步进行中',
		cancelButton: '取消同步',
		progressText: '正在同步文件',
		fileOp: {

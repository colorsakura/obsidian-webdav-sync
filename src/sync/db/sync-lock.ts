...
export class SyncLock {
	...
	async acquire(): Promise<boolean> {
		...
		for (let attempt = 0; attempt < MAX_ACQUIRE_ATTEMPTS; attempt++) {
			try {
				const content = await this.webdav.getFileContents(this.lockPath, {
					format: 'text',
				})
				const lock: LockData =
			...
			// 回读验证
			try {
				const verifyContent = await this.webdav.getFileContents(this.lockPath, {
					format: 'text',
				})
				const verify: LockData =
					typeof verifyContent === 'string'

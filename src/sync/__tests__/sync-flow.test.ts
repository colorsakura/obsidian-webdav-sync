...
function createRealTaskFactory(): TaskFactory {
  ...
  return {
    ...
    createPushTask: (o: TaskOptions) =>
      new PushTask({ ...commonOpts, ...o }),
    createConflictResolveTask: (o: ConflictTaskOptions) =>
      new ConflictResolveTask({ ...commonOpts, ...o }),
    createNoopTask: (o: TaskOptions) =>
      new NoopTask({ ...commonOpts, ...o }),
...
describe('Sync Flow Integration (DB-based)', () => {
  it('首次同步：100 个本地文件 → 全部 Push', async () => {
    ...
    const tasks = await twoWayDecider(await makeInput(localDB, remoteDB, lastSyncDB))
  ...
  it('目录同步：本地新建目录 → MkdirRemote 任务', async () => {
    ...
    localDB.upsertFile({ path: 'new-folder/sub', mtime: 0, size: 0, hash: '', isDir: 1 })

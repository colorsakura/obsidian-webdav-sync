...
export async function twoWayDecider(input: SyncDecisionInput): Promise<BaseTask[]> {
  ...
  const opts = (path: string): TaskOptions => ({ remotePath: path, localPath: path, remoteBaseDir })
      ...
      if (local && remote) {
        if (localChanged && remoteChanged) {
          ...
          if (local.size > maxFileSize || remote.size > maxFileSize) {
            ...
            tasks.push(createFileTooLargeTask(p, maxFileSize, taskFactory, remoteBaseDir, local, remote))
          } else {
          ...
          if (remote.size > maxFileSize) {
            logger.debug(`Skip (too large) — "${p}"`)
            tasks.push(createFileTooLargeTask(p, maxFileSize, taskFactory, remoteBaseDir, undefined, remote))
          } else {
        ...
        if (!localChanged) {
          ...
          if (local.size > maxFileSize) {
            logger.debug(`Skip (too large) — "${p}"`)
            tasks.push(createFileTooLargeTask(p, maxFileSize, taskFactory, remoteBaseDir, local))
          } else {
  ...
  const fileTasks = tasks.filter(t => !removeTypes.has((t as any).type) && !mkdirTypes.has((t as any).type))
